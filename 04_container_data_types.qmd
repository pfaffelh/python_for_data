...
print(sorted(l))
```
Assume you want to sort a `list` not as usual. In this situation, you will find out about the `key`-parameter in `sorted`. It takes a function, we sorts the list according to the values of this function. Here, we use `lambda` to define short anonymous functions inline: `lambda x: expression` creates a function that takes `x` and returns `expression` (see also @sec-functions in Chapter 3). As an example, assume we want to sort a list of `int`s according to their abolute value:
```{python}
l = [5, -4, 0, 7, -3, 2]
...
## Combining lists (`zip`, `enumerate`) {#sec-zip}
