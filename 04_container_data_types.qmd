...
A useful function that takes a list of strings and combines them into a single string is `str.join` (already mentioned briefly in @sec-strings). It is called on the separator string and takes the list as argument.
...
**Exercise 2**
Write a function `is_palindrome(s)` that returns `True` if the string `s` reads the same forwards and backwards (e.g. `"racecar"`), and `False` otherwise. Ignore upper/lower case. (Hint: you can reverse a string using slicing.)
...
**Exercise 9**
Assume you have `a = [10, 20, 30, 40]` and `mask = [True, False, True, False]`, you might want obtain a sublist of `a` which only contains the positions where `mask` has `True` (without using other libraries). (You will probably need the `enumerate`- or `zip`-function here.)
