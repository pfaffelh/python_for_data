...
However, we are working with Jupyter notebooks here, so we cannot run `pytest` on separate files. Although there are workarounds (the package `ipytest`), we will only go through some basics here. This means we are introducing `assert`. 
...
## Interacting with the environment (`sys`)
