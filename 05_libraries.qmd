...
As in all programming languages, not all python programs come in a single file. If a Python project spans multiple files, an `import` becomes necessary. In Python, there is syntax for `import`s as follows:
...
# !pip3 install numpy
```
and then, `import numpy` will work. We will see examples for this e.g. in @sec-loading-files. 
...
## Dates and times of day (`datetime`) {#sec-datetime}
...
```
The reason is that computers in general use a base of 2 for all computations, and 0.3 has a bad representation in this system. While this small **rounding error** is acceptable often, it is not in some contexts (above all, finance). In this case, we can use the `decimal` library. Within `decimal`, a base of 10 is used, but calculations are slower than for `float`s. 
...
for file in os.listdir("."):
    if file.endswith(".ipynb"):  # .endswith() checks if a string ends with the given suffix
        print(file)
```

```{python}
# Print folders and files (only first 5 entries of os.walk)
for i, (root, dirs, files) in enumerate(os.walk(".")):  # enumerate() gives pairs (index, element) for iteration
    if i >= 5:
        print("... (truncated)")
        break
    print("ROOT:", root)
