...
Often, we are given strings such as `"21.4.26, 12:15"`, and want to convert this into a `datetime` object. For this, we have `datetime.strptime`:
```{python}
dt = datetime.strptime("21.4.26, 12:15", "%y.%m.%d, %H:%M")
print(s + f" is converted to {dt}.")
```
(For the correct way to place %, y, m, d etc., it is in fact best to consult your favourite AI.) 

Reversely, we want to print a `datetime`-object in a certain way. For this, we have `datetime.strftime`:
```{python}
