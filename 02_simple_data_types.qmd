...
There are exactly two **boolean** values, known as `True` and `False`. You can compute with them using `not`, `and`, `or` and `^` (xor). In addition, `True` and `False` can be the result of a condition, such as the expression `1 == 2` (see also @sec-equality on the difference between `=`, `==`, and `is`).
...
- **Indexing**: You can access individual characters of a string by position, starting at `0`. So, for `s = "Python"`, `s[0]` is `"P"` (the first letter), `s[1]` is `"y"` (the second letter), and so on. Negative indices count from the end: `s[-1]` is `"n"` (the last letter). We will come back to indexing (and slicing) in @sec-list.
...
* `f"{n:,}"`: integer with thousands separator (here the comma `,`).
