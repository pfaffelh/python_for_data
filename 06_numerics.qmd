...
# np.abs() computes the absolute value elementwise
...
# np.sum() and np.max() are aggregation functions, explained below
...
**Exercise 9**
Use `scipy.optimize.root_scalar` to find all three roots of $f(x) = x^3 - 6x^2 + 11x - 6$ on the interval $[0, 4]$. Hint: the roots are integers. Plot the function and mark the roots.
